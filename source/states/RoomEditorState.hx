...
class RoomEditorState extends FlxState {
    ...
    function buildDraggablePropertyWindow() {
        ...
        // Godot Grid inputs now isolated entirely to the Anims tab
    ...
    function updateGhostCursor() {
        ...
        if (spriteStr != lastSpriteStr || sx != lastScaleX || sy != lastScaleY) {
            ...
            // Wipe offsets so preview strictly respects origin (x,y) XML placement mapping
    ...
    function placeObject(x:Float, y:Float) {
        ...
        placedSprite.cameras = [workspaceCam]; // Ensure layering under UI HUD
    ...
    function refreshTilePalette() {
        ...
        for (i in start...end) {
            ...
            if (openfl.utils.Assets.exists("assets/" + t.path + ".png")) {
                ...
                prev.setGraphicSize(32, 32); prev.updateHitbox(); prev.cameras = [uiCamera]; // FORCE UI CAMERA
    ...
    function parseLoadedXML(xmlStr:String, clearOld:Bool = true) {
        ...
        editingElement = null; xmlStr = StringTools.replace(xmlStr, "<!DOCTYPE lacie-engine-room>", "");
