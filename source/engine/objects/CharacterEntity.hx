...
class CharacterEntity extends WorldObject {
    ...
    // Spatial Breadcrumb configuration
    public var positionHistory:Array<FlxPoint> = [];
    public var maxHistory:Int = 40;
    public var historySpacing:Float = 2.0; // Must move 2 pixels to drop a coordinate
        ...
        if (LilyAssets.fileExists(xmlPath)) {
            ...
            if (rawXml.indexOf("<character") != -1) {
                ...
                if (xml != null && xml.nodeName == "character") {
                    ...
                    if (parsed.has.flipX) this.flipX = parsed.att.flipX == "true";
                    if (parsed.has.flipY) this.flipY = parsed.att.flipY == "true";
                    ...
                    if (parsed.hasNode.anim) {
                        for (animNode in parsed.nodes.anim) {
                            ...
                            if (!loadedFrames.exists(aSprite)) {
                                ...
                                if (atlas != null) loadedFrames.set(aSprite, atlas);
                            }
                        }
                    }
    ...
    function setupStaticHitbox() {
        ...
        if (hitboxGraphic == null) hitboxGraphic = new FlxSprite();
        ...
        if (animData.exists(animName)) {
            ...
            if (isNewAnim) {
                ...
                if (currentSpritePath != data.spritePath && loadedFrames.exists(data.spritePath)) {
                    ...
                    for (anim in oldAnims) animation.remove(anim);
    ...
    public function updateAnimations():Void {
        if (Math.abs(velocity.x) > 5 || Math.abs(velocity.y) > 5) {
            if (Math.abs(velocity.x) > Math.abs(velocity.y)) {
                if (velocity.x > 0) currentFacing = RIGHT;
                else currentFacing = LEFT;
            } else {
                if (velocity.y > 0) currentFacing = DOWN;
                else currentFacing = UP;
            }
        }
        ...
        var animName = "";
        switch (currentState) {
            case Walking: animName = "walk" + faceStr;
            case Running: animName = "run" + faceStr;
            case Standing, Idle: animName = "idle" + faceStr;
        }
