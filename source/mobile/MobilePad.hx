...
class MobilePad extends MobileInputHandler {
    ...
    public var lastDPadName:String = "NONE";
    public var lastActionName:String = "NONE";
	...
	public function createVirtualButton(x:Float, y:Float, framePath:String, ?scale:Float = 1.0, ?ColorS:Int = 0xFFFFFF, ?returned:String):MobileButton {
		...
		final path:String = LilyAssets.getPath(MobileConfig.mobileFolderPath + 'MobilePad/Textures/$framePath.png');
		if(LilyAssets.fileExists(path))
			frames = FlxGraphic.fromBitmapData(FileSystem.getBitmapData(path));
		else
			frames = FlxGraphic.fromBitmapData(FileSystem.getBitmapData("assets/" + MobileConfig.mobileFolderPath + 'MobilePad/Textures/default.png'));
