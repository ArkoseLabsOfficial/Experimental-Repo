...
class SettingsScreen extends SubStateBackend {
    ...
    override public function create():Void {
        ...
        if (useTitle) {
            menuFrame.setTitle(Language.GetCaption(options[0] != null ? "system.settings.category." + currentMenuId : "Settings"));
        }
    ...
    function changeSelection(change:Int):Void {
        curSelected += change;
        if (curSelected < 0) curSelected = visualItems.length - 1;
        if (curSelected >= visualItems.length) curSelected = 0;
        updateHighlight();
    }
    ...
    function acceptSelection():Void {
        ...
        if (currentMenuId == "main") {
            ...
            else openSubState(new SettingsScreen(opt.target, fromPause)); 
        } else {
    ...
    function adjustOption(opt:XmlOption, dir:Int):Void {
        ...
        if (opt.onChanged != null) opt.onChanged(newVal);
...
class SettingsVisualEntry extends MenuVisualEntry {
    ...
    public function new(X:Float, Y:Float, opt:XmlOption, width:Float, height:Float, scale:Float) {
        ...
        optionLabel = new FlxText(0, 0, 0, Language.GetCaption(opt.label), ts);
        
        if (opt.type == "category" || opt.type == "button") {
            optionLabel.x = (width - optionLabel.width) / 2;
        } else {
            optionLabel.x = 15 * scale;
        }
        
        optionLabel.y = (height - optionLabel.height) / 2;
        add(optionLabel);
    ...
    public function updateVisuals(val:Dynamic):Void {
        if (optData.type == "button" || optData.type == "category") return;
