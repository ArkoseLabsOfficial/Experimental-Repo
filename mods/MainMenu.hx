...
class MainMenu extends StateBackend {
    ...
    override public function create():Void {
        ...
        if (hasSaveFile) {
            menuDefinitions.push({ transKey: "system.menu.loadgame", action: function(d:Int) { openSubState(new SaveLoadSubState(false, true)); } });
        }
        menuDefinitions.push({ transKey: "system.menu.newgame", action: function(d:Int) { SaveManager.reset(); FlxG.switchState(new PlayState("room")); } });
        menuDefinitions.push({ transKey: "system.menu.debugroom", action: function(d:Int) { FlxG.switchState(new RoomEditorState()); } });
        menuDefinitions.push({ transKey: "system.menu.settings", action: function(d:Int) { openSubState(new SettingsScreen()); } });
        menuDefinitions.push({ transKey: "system.menu.website.translator", action: function(d:Int) {} });
        menuDefinitions.push({ transKey: "system.menu.quit", action: function(d:Int) { System.exit(0); } });
    ...
    override public function update(elapsed:Float):Void {
        ...
        if (visualItems.length > 0 && Controls.ACCEPT_P) {
    ...
    public function changeSelection(change:Int = 0):Void {
        ...
        if (selectedIndex < 0) selectedIndex = visualItems.length - 1;
        if (selectedIndex >= visualItems.length) selectedIndex = 0;
...
class MainMenuVisualEntry extends MenuVisualEntry {
    ...
    public function new(X:Float, Y:Float, transKey:String, frameWidth:Float, cellHeight:Float) {
        ...
        optionLabel = new FlxText(0, 0, frameWidth, Language.GetCaption(transKey), ts);
    ...
    public function refreshText():Void {
        optionLabel.text = Language.GetCaption(transKey);
    }
}